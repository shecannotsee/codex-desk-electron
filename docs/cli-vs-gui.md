...
| 运行中状态面板 | 不支持 | 仅 GUI | 聊天区显示临时状态和当前步骤 |
| 原始 JSON 面板 | 部分支持 | 增强 | GUI 做归档浏览 |
| 选中文本右键复制 | 依赖终端 | 仅 GUI | 聊天区和运行面板统一复制入口 |
...
| 缩放百分比提示 | 不支持 | 仅 GUI | `Alt+=/-/0` 时顶部提示当前比例 |
| 切换会话自动到底部 | 不支持 | 仅 GUI | 直接对齐到最新消息 |
| 外部链接默认浏览器 | 手动打开 | 增强 | 不在应用内开额外窗口 |
...
4. GUI 会在回复生成期间额外提供临时状态区，而 CLI 只能看进程输出。
