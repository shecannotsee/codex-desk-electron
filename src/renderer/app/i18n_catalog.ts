...
const I18N: Record<Language, Record<string, string>> = {
  'zh-CN': {
    ...
    securityEntryHint: '用主密码加密保存已接入通讯工具的敏感凭据，锁定后需要解锁才能使用。',
    securityPaneTitle: '凭据保护',
    securityPaneHint: '主密码只用于保护本地保存的通讯凭据。设置后，已接入通讯工具的敏感凭据会以加密形式保存在当前设备；应用重启后默认需要先解锁，通知、远程对话和测试才可继续使用。',
    ...
    securityUnlockNow: '前往解锁',
    securityLockedHint: '当前已锁定。解锁后才可查看、修改或使用已保存的通讯凭据；已启用的通知、远程对话和测试也会在锁定时暂停。',
    securityUnlockedHint: '当前已解锁。你可以修改已保存的通讯凭据，也可以重新设置主密码。',
    securitySetPasswordHint: '设置主密码后，后续保存的通讯凭据会改为加密保存。',
    securityRuntimeLockedTitle: '通讯功能已暂停',
    securityRuntimeLockedHint: '当前凭据已锁定。已启用的通知、远程对话和测试发送已暂停，解锁后才会恢复。',
    securityRuntimeLockedNotice: '通讯凭据当前处于锁定状态，通知、远程对话和测试发送已暂停。请到 设置 > 凭据保护 解锁。',
    ...
    securityUnlockSuccess: '通讯凭据已解锁',
    securityLockSuccess: '通讯凭据已锁定',
  ...
  'en-US': {
    ...
    securityEntryHint: 'Encrypt sensitive credentials for connected messaging providers with a master password. Unlocking is required before use.',
    securityPaneTitle: 'Credential Vault',
    securityPaneHint: 'The master password protects only the messaging credentials saved on this device. After setup, sensitive credentials for connected providers are stored locally in encrypted form; after restart, unlock is required before notifications, remote chat, and tests can continue.',
    ...
    securityLockedHint: 'Credentials are locked. Unlock them before viewing, editing, or using saved messaging credentials; enabled notifications, remote chat, and tests are also paused while locked.',
    securityUnlockedHint: 'Credentials are unlocked. You can edit saved messaging credentials or replace the master password.',
    securitySetPasswordHint: 'After the master password is set, saved messaging credentials will be stored in encrypted form.',
    securityRuntimeLockedTitle: 'Messaging Paused',
    securityRuntimeLockedHint: 'Credentials are locked. Enabled notifications, remote chat, and test sends are paused until you unlock them.',
    securityRuntimeLockedNotice: 'Messaging credentials are locked. Notifications, remote chat, and test sends are paused. Open Settings > Credential Vault to unlock.',
    ...
    securityUnlockSuccess: 'Messaging credentials unlocked',
    securityLockSuccess: 'Messaging credentials locked',
    providerTelegram: 'Telegram',
    showSecret: 'Show',
    hideSecret: 'Hide',
